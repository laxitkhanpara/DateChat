...
export default function ChatBox() {
    ...
    if (isMessagesLoading) return <p style={{ textAlign: "center", width: "100%" }}>Loading chats...</p>;
        ...
        <Stack gap={4} className='chat-box'>
            ...
            <Stack gap={3} className='messages'>
                {messages && messages.map((message, index) => (
                    <Stack key={index} className={`${message?.senderId === user?.user[0]._id ? "message self align-self-end flex-grow-0" : "message  align-self-start flex-grow-0"}`} ref={scroll}>
